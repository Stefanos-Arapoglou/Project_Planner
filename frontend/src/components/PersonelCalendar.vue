  ...
  const promises = this.projects.map(async (project) => {
    try {
      ...
      console.log(`Project ${project.project_code}:`, {
        assigned: project.assigned_personel,
        expected: project.expected_personel,
...
// In PersonnelCalendar.vue - UPDATED method
getProjectsOnDate(person, date) {
  ...
  return person.projects.filter(project => {
    ...
    // Find the enriched project data and merge it
    const enrichedProject = this.projects.find(p => p.project_id === assignedProject.project_id);
    return enrichedProject ? { ...assignedProject, ...enrichedProject } : assignedProject;
  });
},
...
// In PersonnelCalendar.vue - IMPROVED method
getProjectTooltip(project) {
  // Use the actual assigned personnel count from our enriched data
...
async openProjectCardFromPersonel(project) {
  try {
    ...
    console.error('Error fetching project details from PersonelInfo:', error);
